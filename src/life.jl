...
function virtual_expansion(starting_array, radius)
    ...
    left = vcat(starting_array[nh:end,nw:end], starting_array[:,nw:end], starting_array[1:radius, nw:end])
    right = vcat(starting_array[nh:end, 1:radius], starting_array[:,1:radius], starting_array[1:radius,1:radius])
    middle = vcat(starting_array[nh:end,:], starting_array, starting_array[1:radius,:])

    hcat(left, middle, right)
end
...
function life_evolution(starting_array, born, survive, radius)
    ...
    for i = 1:height-radius+1, j = 1:width-radius+1
        output[i, j] = life_application(virtual_output[i:i+radius+1, j:j+radius+1], born, survive)
    end
    output
end
